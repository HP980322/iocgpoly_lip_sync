...
class LIPSYNC2D_PoseAssetsAnimator:
    ...
    def insert_keyframe_points(
        ...
        target = self.target_obj if hasattr(self, "target_obj") else bpy.context.active_object
        
        # 2. Blender 5.0 Requirement: Assign action to object before modifying F-curves
        if target and target.animation_data:
            if target.animation_data.action != pose_action:
                target.animation_data.action = pose_action

        for fcurve in self.channelbag.fcurves:
            # 3. Use the 5.0 compliant method to find/ensure the F-curve
            pose_asset_fcurve = pose_action.fcurve_ensure_for_datablock(
                datablock=target, 
                data_path=fcurve.data_path, 
                index=fcurve.array_index
            )

            if pose_asset_fcurve is None or len(pose_asset_fcurve.keyframe_points) == 0:
                continue
